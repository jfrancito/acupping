...
class UserController extends Controller {
	...
	public function actionAgregarUsuario($idopcion, Request $request) {
		...
		if ($_POST) {
			...
			$cabecera->usuario_solomon_id = '1CIX00000001';
			$cabecera->fecha_crea = $this->fechaactual;
			$cabecera->usuario_crea = Session::get('usuario')->usuario_solomon_id;
			$cabecera->save();

			return Redirect::to('/gestion-de-usuarios/' . $idopcion)->with('bienhecho', 'Usuario ' . $nombres . ' registrado con exito');
	...
	public function actionModificarUsuario($idopcion, $idusuario, Request $request) {
		...
		if ($_POST) {
			...
			$rol = DB::table('Rols')->where('id', '<>', $this->prefijomaestro . '00000001')->pluck('nombre', 'id')->toArray();
			$comborol = array($usuario->rol_id => $usuario->roles->nombre) + $rol;
			$funcion = $this;
